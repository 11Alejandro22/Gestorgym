<x-layouts.app>
        ...
        <flux:heading class="mb-4 mt-6" size="xl">Agrega un nuevo Cliente</flux:heading>
        <flux:separator/>
        ...
        <div class="bg-gray px-6 py-8 shadow-lg rounded-lg">
            ...
            <form action="{{route('admin.clients.store')}}" method="POST" class="space-y-6">
                ...
                <flux:input label="DNI"      name="DNI"        value="{{old('DNI')}}"        placeholder="Escriba un DNI"   icon="identification" type="text"  ></flux:imput>

                <flux:input label="Telefono" name="phone"      value="{{old('phone')}}"      placeholder="(9999) 999-999"   icon="phone" type="text"  mask="(999) 9999-999"  />
                
                <flux:input label="Email"    name="email"      value="{{old('email')}}"      placeholder="Email@correo.com" icon="envelope" />
                    ...
                    @foreach ($schedules as $schedule)
                        ...
                        <div class="scale-100 w-full [@media(min-width:450px)]:w-[350px] shadow-lg p-4 bg-white border border-gray-200 rounded-lg sm:p-8 dark:bg-gray-800 dark:border-gray-700">
                            ...
                            <div class="w-full max-w-sm flex justify-center items-center mb-4">
                                <flux:checkbox class="scale-150 cursor-pointer" 
                                name="category_schedules[]"
                                :value="$schedule->id"
                                :checked="in_array($schedule->id, old('category_schedules', []))"/>
                            </div>
